...
class DeepseekSparseSWAMetadataBuilder(AttentionMetadataBuilder):
    ...
    def build(
        ...
        # DSpark draft block (DSv4-style drafts only; the qwen3-backbone GLM
        # draft never routes here): every query attends to the trailing context
        # window PLUS the whole query block, so the decode slice needs the wider
        # non-causal index list in its own buffer. The causal launch above still
        # provides the prefill slice.
        if non_causal and num_decode_tokens > 0:
